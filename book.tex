...
We can compensate for the additional factor of $2^{-32} \pmod b$ by pre-multiplying all numbers by $2^{32}$, or by multiplying by $(2^{32})^{\text{\#multiplications}} \pmod b$ at the very end.
In code:
...
On the other hand, both the float-based modular multiplication and the Montgomery multiplication are in principle easy to vectorize. The main annoyance comes from the fact that the divisibility check by $p$ starts to become a bottleneck, in comparison to the scalar code where it was only about a 5\% speed loss.
