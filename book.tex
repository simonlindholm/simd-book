...
% only gcc, measurements on i5-4200U (AVX2)
...
0.434s! That's better, and enough to get Accepted on the problem. With an \verb@avx2@ target pragma it goes down further to 0.225s. In fact, even with auto-vectorization disabled this trick helps performance, giving 0.853s.
...
} so we can look only at ranges with upper bound less than $p^\alpha$. Here's an initial attempt:
